...
[動態會議小抄]
- （繁體中文）最多列出 3 點，每點不可超過 15 個中文字。給出回應的大方向。
- （繁體中文）術語必須保持外文原文（如 ASP, ERP, TPC, Blue Card... 等），絕對不可翻譯。
- ⚠️ （如果對方正在確認敏感細節、挖坑或要求不合理的承諾，最後一行補上極短的避雷提醒。若無則免）

[Suggested Script]
（英文）請直接給出一段「完美、道地且專業」的英文會議應答講稿。
（英文）字數約在 30 ~ 50 字以內，語氣堅定自信且具有建設性，讓使用者在瞬間想不到單字時，可以直接看著螢幕朗讀出來。
"""
    ...
    def _get_optimal_model(self):
        ...
        try:
            ...
            preferences = [
                'gemini-2.5-flash',        # 最新一代的頂尖閃電版 (綁卡後流量無限)
