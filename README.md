...
* **Dual-Track Apple Silicon Transcriber**: 
  It utilizes `MLX-Whisper` directly on the Mac NPU, safely separating hardware microphones (You) and Virtual Audio loops like BlackHole (Them) without system crashes. The live model is auto-selected by hardware: fanless Macs (MacBook Air) get the lighter `whisper-medium-mlx` (same clean-speech accuracy, ~1.5x faster, runs cooler), other Macs get `whisper-large-v3-turbo`. Both are multilingual, so mixed Chinese/English (code-switching) stays accurate. Set `WHISPER_MODEL` to override.
...
|----------|---------|-------------|
| `WHISPER_MODEL` | _auto by hardware_ | The live speech-to-text model. **Unset → auto-selected**: fanless Macs (MacBook Air, detected via `system_profiler`) use `whisper-medium-mlx`; other Macs use `whisper-large-v3-turbo`. Set an explicit id to override — e.g. `mlx-community/whisper-large-v3-turbo` to force the larger model on an Air, or `mlx-community/whisper-small-mlx` for the smallest footprint. Changing this needs a restart (mlx caches one model per process), so both mics always share it. Offline `retranscribe.py` always uses the larger default for best quality. |
